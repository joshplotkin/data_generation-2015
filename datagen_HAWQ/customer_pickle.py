...
class Customer:
	...
	def __init__(self):
		self.fake = Faker()
		self.fake.seed(int(np.random.uniform(-1000000,1000000)))
		self.all_profiles = pickle.load(open('all_profiles.pickle', 'rb'))
		self.cities = pickle.load(open('cities.pickle', 'rb'))
		self.age_gender = pickle.load(open('age_gender.pickle', 'rb'))
	...
	def generate_transactions(self):
		...
		# output = []
		transactions = pickle.load(open(profile_pickle, 'rb')).sample_from()
		return [self.customer_attr_list + '|' + t for t in transactions]
